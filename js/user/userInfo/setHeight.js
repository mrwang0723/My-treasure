...
var heightDefault=parseFloat(location.hash.replace('#','')) || 170
$(function () {
    new Vue({
        el: '#height',
        data: {
        ...
        mounted: function () {
            ...
            this.$nextTick(function () {
                ...
                $('.edit-set-content-num').css({'transform':'translate(0,' + (heightDefault-240)*$('.edit-set-content-num li').eq(0).outerHeight(true)/10 + 'px)','-webkit-trasform' : 'translate(' + (heightDefault-240)*$('.edit-set-content-num li').eq(0).outerHeight(true)/10 + 'px,0)'});
        ...
        methods: {
            setHeight: function () {
                var data = {
                    body_height: $('.edit-show-num s').html() + 'cm'
                }
                var _this = this;
    ...
    $(document).on('touchstart', '.edit-set-left,.edit-set-right', function () {
        ...
        ;
        $('.edit-show-num s').html(flag ? text - parseInt(Math.abs(position / (aWidth / 10))) : text + parseInt(Math.abs(position / (aWidth / 10))))
    ...
    $(document).on('touchstart', '.edit-set-content', function () {
        ...
        var flag = $(event.target).parents().hasClass('edit-height-box'),
            ...
            text = parseInt($('.edit-set-content-num li').eq(0).text()),
            sT = Date.now(),
            mT = 0;
        ...
        $(document).on('touchmove', '.edit-set-content', function () {
            ...
            numBox.html(flag ? text - parseInt(Math.abs(mPosition / (aWidth / 10))) : text + parseInt(Math.abs(mPosition / (aWidth / 10))))
        ...
        $(document).off('touchend', '.edit-set-content').on('touchend', '.edit-set-content', function () {
            ...
            if (eT - mT < 50) {
                timer.timer = setInterval(function () {
                    ...
                    $('.edit-show-num s').html(flag ? text - parseInt(Math.abs(ePosition / (aWidth / 10))) : text + parseInt(Math.abs(ePosition / (aWidth / 10))))
                }, 10);
            }
        });
    });
